...
vk.setOptions({ token: 'Токен', pollingGroupId: айди });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club209070587\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club178535452\|(.*)\]/ig, '').trim();
...
cmd.hear(/^(?:zz)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId !== 469284392) return;
...
👨‍💻 Администратор бота - Дамей Алиев.`);
});
...
cmd.hear(/^(?:помощь)\s(.*)$/i, async (message, bot) => {
	...
	if(message.args[1] === 'админ')
	{
		return message.send(`🥰 Администратором бота является - Дамей Алиев • по вопросам писать ему •`);
	}
...
cmd.hear(/^(?:продать)\s(.*)\s?(.*)?$/i, async (message, bot) => {
	...
	if(/квартира/i.test(message.args[1].toLowerCase()))
	{
...
cmd.hear(/^(?:репорт|реп|rep|жалоба)\s([^]+)$/i, async (message, bot) => {
	...
	vk.api.messages.send({ user_id: 469284392, forward_messages: message.id, message: `Player id: ${message.user.uid}` }).then(() => {
